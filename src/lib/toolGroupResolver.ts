 ...
 */
export function getToolGroupId(id: string): string | null {
    // Terminal & shell
    if (
    ...
    // HTTP direct
    if (id === "http_request") return "http";

    // Recherche web
    if (id === "search_web") return "search_web";

    // Scraping
    if (id === "scrape_url") return "scrape_url";
    ...
    if (["context7-search", "context7-docs"].includes(id)) return "context7";

    // MCP
    if (["create_mcp_server", "start_mcp_server", "call_mcp_tool", "list_mcp_servers"].includes(id)) return "mcp";

    // Mémoire conversations
    if (id === "search_conversation") return "memory";

    // Planification
    if (["get_plan", "save_plan", "set_todo", "check_todo"].includes(id)) return "planning";

    // Profil utilisateur
    if (id === "save_fact") return "profile";
